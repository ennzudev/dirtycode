...
class Construccion:
    ...
    #!                                                                      ¿Qué metodo de refactorización se aplicó aquí? R: Extract Method
...
class Zombie:
    ...
    def info(self):
        ...
        Último cerebro: {}'''.format(
            self.__nombre,
            self.__tipo.name,
            self.tiempo_desde_ultimo_alimento()#!                                               Extract method R: Listo
        )

    
    def tiempo_desde_ultimo_alimento(self):
        ''' Tiempo que ha pasado sin alimento nuestro zombie '''
    ...
    def comparacion_alimento(self, segundos) -> bool:
        return datetime.now() - self.__ultimo_alimento > timedelta(seconds=segundos)

    @property
    def estado_de_animo(self):
        ''' Regresa el estado de ánimo de nuestro zombie basado en el tiempo
            que ha pasado sin comer '''
            #!                                                                               Aplicar inline variable R: Listo
        if self.comparacion_alimento(10):
            return 'agresivo'
        elif self.comparacion_alimento(20):
            return 'enfadado'
        elif self.comparacion_alimento(5):
            return 'normal'
        else:
            return 'contento'
    
    #!                                                                                           Aplicar inline method R: Listo
    ...
    #!                                                                                           Aplicar extract variable R: Listo
    @property
    def peligrosidad(self):
        ''' Regresa el nivel de peligrosidad del zombie en tres niveles
            alta, media o baja '''
        tipo = self.__tipo == TipoZombie.ZOMBIEINSTEIN or self.__tipo == TipoZombie.ZOMBIEPULTA
        animo_normal = self.estado_de_animo == 'contento' or self.estado_de_animo == 'normal'
        cerebros_devorados = self.__cerebros_devorados >= 1

        if not tipo and animo_normal and cerebros_devorados:
            return 'baja'
        elif not tipo and not animo_normal and cerebros_devorados:
            return 'media'
        else:
            return 'alta'
    ...
    #!                                                                                   Aplicar replace temp with query R: Listo
    def ir(self, lugar, tiempo_del_recorrido: timedelta):
        ...
        tiempo_restante = self.tiempo_de_vida_restante
        if tiempo_restante >= tiempo_del_recorrido:
            return f'Adelante, puede ir al {lugar}', True
        else:
            return f'No se recomienda ir al {lugar}. Desaparecerá antes', False

    
    #!                                                                                   Aplicar Split temporary variable R: Listo
